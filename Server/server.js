...
let urlDBMemory = "mongodb+srv://Testuser2:Test123@marissareiser-gis21.8i9as.mongodb.net/Memory?retryWrites=true&w=majority"; //richtige DB und Collection MemoryKarten verwenden
let urlDBHighscore = ""; //richtige DB und Collection Highscore verwenden
...
async function handleRequest(_request, _response) {
    ...
    if (_request.url) {
        ...
        let memoryKarte = { bild: url.query.bild + "", url: url.query.url + "" };
        //Pfad um die ScoreDaten in DB zu speichern -->Button auf Spielergebnisseite (Bestätigen und senden)
        if (pathname == "/Abschicken") {
            let antwort = await HighscoreDatenSpeichern(urlDBHighscore, highscore);
            console.log(antwort);
        ...
        //Pfad wenn man ein Bild in die DB hinzufügen möchte
            ...
            let memoryKarten = await bilderHinzufuegen(urlDBMemory, memoryKarte);
        ...
        //Pfad wenn man ein Bild aus der DB löschen möchte
            ...
            let memoryKarten = await bildLoeschen(urlDBMemory);
            console.log(memoryKarten);
            _response.write(JSON.stringify(memoryKarten));
        }
    }
...
async function HighscoreDatenSpeichern(_url, _highscore) {
    ...
    let ausgabe = "Hinzugefügt"; //Zur Überprüfung ob gespeichert wurde
    return ausgabe;
}
//Funktion Bilder Löschen auf der Admin Seite
async function bildLoeschen(_url, _name) {
    let options = { useNewUrlParser: true, useUnifiedTopology: true };
    let mongoClient = new Mongo.MongoClient(_url, options);
    await mongoClient.connect();
